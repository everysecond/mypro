...
$("input[name='contractCycle']").change(function(){
    if($(this).val()>0){
        var startTs = new Date($("#startTs").val()),a =($(this).val()*1);
        if(startTs.getMonth()+a == 1 && startTs.getDate()-1>28){
            startTs.setDate(27);
            startTs.setMonth(startTs.getMonth()+a);
        }else{
            startTs.setMonth(startTs.getMonth()+a);
            startTs.setDate(startTs.getDate()-1);
        }
        $("#endTs").val(startTs.Format("yyyy-MM-dd") );
    }
});

$("#startTs").change(function(){
    if($("input[name='contractCycle']").val()>0){
        var startTs = new Date($("#startTs").val()),a =($("input[name='contractCycle']").val()*1);
        if(startTs.getMonth()+a == 1 && startTs.getDate()-1>28){
            startTs.setDate(27);
            startTs.setMonth(startTs.getMonth()+a);
        }else{
...
var contactBsSuggest = $("#cusname").bsSuggest({
    ...
    showHeader: false,
    url: '/rpms/resourceContract/findSupplierBySearch?code=utf-8&extras=1&name=',
    ...
    console.log(globaldata[keyword.id - 1].providerName);
    $('#supplierId').val(globaldata[keyword.id - 1].id);
    $('#supplierName').val(globaldata[keyword.id - 1].providerName);
})
