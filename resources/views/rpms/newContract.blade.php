                        ...
                        <fieldset>
                            ...
                            <table class="table-edit" style="color: #000000;" bgcolor="#FFFFFF" border="1"
                                ...
                                @if($contract!="" && $contract->oldContractNo)
                                    <tr>
                                        <td align="right" width="15%">原始合同编号：</td>
                                        <td colspan="3">
                                            @if($contract!="" && $contract->oldContractNo)
                                                <a href='/rpms/resourceContract/contractConfirm/{{$oldContract->id}}?type=detail&&contractId={{$oldContract->id}}'
                                                   class="fontred ml3" target="_blank">{{$contract->oldContractNo}}
                                                </a>
                                            @endif
                                ...
                                <tr>
                                    ...
                                    <td width="30%">
                                        <input class="form-control validateDoing ml3" type="number" min="1" max="1000"
                                               name="contractCycle" placeholder="请输入合同周期(月)" id="contractCycle"
                                               value="@if($contract!="" && $contract->contractCycle){{$contract->contractCycle}}@elseif($contract == ""||!$contract->contractCycle){{$defaultValue["contractCycle"]}}@endif" style="width: 100%;"/>
                                    </td>
                                    ...
                                    <td >
                                        <select name="currencyType" id="currencyType"
                                            ...
                                            @foreach($currencyList as $key=> $currency)
                                                <option value="{{$key}}"
                                                        @if(($contract!="" && $contract->currencyType == $key)|| (($contract=="" ||!$contract->currencyType) &&$key == $defaultValue["currencyType"])) selected @endif>
                                                    {{$currency}}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    ...
                                    <td >
                                        <select name="balanceCycle" id="balanceCycle"
                                            ...
                                            @foreach($balanceList as $key=> $balance)
                                                <option value="{{$key}}"
                                                        @if(($contract!="" && $contract->balanceCycle == $key) || (($contract=="" ||!$contract->balanceCycle) &&$key == $defaultValue["balanceCycle"])) selected @endif>
                                                    {{$balance}}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    ...
                                    <td >
                                        <select name="paymentMode" id="paymentMode"
                                            ...
                                            @foreach($paymentMode as $key=> $mode)
                                                <option value="{{$key}}"
                                                        @if(($contract!="" && $contract->paymentMode == $key) || (($contract=="" ||!$contract->paymentMode) &&$key == $defaultValue["paymentMode"])) selected @endif>
                                                    {{$mode}}</option>
                                            @endforeach
                                        </select>
                                    </td>

                                    <td align="right" >预/后付费(天)：</td>
                                    <td >
                                        <input name="days" id="days" type="text"
                                               class="validateDoing form-control ml3"
                                               placeholder="请输入预付/后付费天数"
                                               value="@if($contract!="" && $contract->days){{$contract->days}}@elseif($contract == ""||!$contract->days){{$defaultValue["days"]}}@endif" >
                                    </td>
                                </tr>
                                <tr><td colspan="4" ><span class="module">资源信息</span></td></tr>
                                <tr><td colspan="4"><a type="reset" class="btn btn-warning btn6 btnred ml15 mb4"
                                                       onclick="pickResource()" style="">
                                            <i class="fa fa-plus mr4" style="color:white;"></i>选择资源产品</a></td></tr>
                                <tr>
                                    <td colspan="4" >
...
<script>
    ...
    $('.btnSub').click(function () {
        ...
        if (!validateMark) {
            $.ajax({
                type: "POST",
                data: {formData,
                    'prodData':$("#prodTable").bootstrapTable('getData')
                },
                ...
                [
                    ...
                    },
                    {
                        ...
                        editable:{
                            ...
                            validate:function(v){
                                ...
                                if($.trim(v) <=0 || parseInt($.trim(v))!=$.trim(v)) return '数量只能是正整数';
                                var val=v.replace('¥','');
                    ...
                    },
                    {
                        ...
                        footerFormatter:function (data) {
                            ...
                            var total= data.reduce(function(sum, row) {
                                return sum + Math.round(row.oneCost*100)/100 * row.amount;
                            }, 0);
                    ...
                    },
                    {
                        ...
                        formatter:function(value,row,index){
                            var operate = operate = '<a title="删除" onclick="delContract(' + row.id + ',' + row.contractId + ')"><i class="fa fa-trash hoverred"></i></a>';
                            if(row.isSpecialLine != "no" && $("#contractId").val()!=""){
                                operate ='<a title="添加记录" onclick="addRecord(' + row.id + ',null)"><i class="fa fa-plus hoverred mr10"></i></a>'+operate;
                            }
                            return operate;
                        },
                    },
            ...
            columns: [
                [
                    ...
                    },
                    {
                        ...
                        align: 'left',
                        formatter:function(value,row,index){
                            var operate = '<a title="删除" onclick="delRecord(' + row.id + ')"><i class="fa fa-trash hoverred"></i></a>';
                            if(row.isSpecialLine != "no"){
                                operate ='<a title="修改记录" onclick="addRecord(' + row.prodId + ','+row.id+')"><i class="fa fa-edit hoverred mr10"></i></a>'+operate;
                            }
                            return operate;
                        },
                    },
    ...
    function delContract(delId, contractId) {
        layer.confirm("是否确认删除?", {title: "提示", btn: ['确定', '取消']}, function () {
            ...
            $.ajax({
                ...
                url: "/rpms/resourceContract/getSpecialCount?contractId=" + $("#contractId").val(),
                success: function (data) {
                    var specialTable = $(".specialTable");
                    if (data.recordCount > 0 || data.prodCount > 0) {
                        specialTable.removeClass("hidden");
                    } else {
                        specialTable.removeClass("hidden");
                        specialTable.addClass("hidden");
                    }
                }
            })
        })
    ...
    function addRecord(prodId,recordId){
        layer.open({
            type: 2,
            title: "添加记录",
            area: ['650px', '450px'],
            shade: 0.2,
            content: ['/rpms/resourceContract/addRecord/'+prodId+"?contractId="+$("#contractId").val()+"&recordId="+recordId]
        });
    }
    ...
    function delRecord(recordId){
        ...
        layer.confirm("是否确认删除?", {title: "提示", btn: ['确定', '取消']}, function () {
            $.ajax({
                ...
                url: '/rpms/resourceContract/delRecord/'+recordId,
                success: function (data) {
                    if (data.status == 'success') {
                        layer.msg('已删除！', {time: 2000});
                        $('#specialRocordTable').bootstrapTable("remove", {field: 'id', values: [recordId]});
                    } else {
                        layer.msg(data.msg, {time: 2000});
                    }
                }
