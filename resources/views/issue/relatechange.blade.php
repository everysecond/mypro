    ...
    <div class="col-sm-6 relateIssue">
        <div class="" style="margin-top: 3px;border:0px;width:760px">
            <form class="form-inline" onsubmit="return false">
                ...
                <input id="issueId" value="{{$issueId}}" type="hidden"/>
            </form>
            <table id="devTable" class="table-no-bordered active"
                   ...
                   data-url="/issue/getRelateChange"
    ...
    function initTable() {
                ...
                [{
                    ...
                    formatter: function (value, row, index) {
                        var s = '<a class="showTitleTips J_menuItem"  menuName="' + row.Id + '"  id="title_' + row.Id + '" ' +
                                '>' + substringLen(row.changeTitle,20) + '</a>';
                        return s;
                    },
                    ...
                    align: 'center',
                    formatter: function (value, row, index) {
                        var s = substringLen(row.changeObject);
                        return s;
                    },
                }, {
        ...
        window.operateEvents = {
            'mouseover .showTitleTips': function (e, value, row, index) {
                custips = layer.tips('<div style="word-wrap: break-word;color: #ffffff">变更标题：' + row.changeTitle + '</div>'
        ...
        $devTable.on('click-cell.bs.table', function ($element, field, value, row) {
            if (field == 'operation') {
                parent.$('#hiddenChangeId').val(row.Id);
                if ($('#hiddenChangeId').val() != '') {
                    layer.confirm('您确定要关联此变更?', {icon: 3, title: '提示'}, function (index) {
                        $.ajax({
                            type: "post",
                            data: {
                                changeId: parent.$('#hiddenChangeId').val(),
                                issueId: parent.$('#issueId').val(),
                                triggerId: parent.$('#issueId').val()
                            },
                            dataType: 'json',
        ...
        layer.confirm('确定要批量关联变更吗?', {
                ...
                function () {
                    ...
                    if (selected.length < 1) {
                        layer.msg('请选择要关联的变更单！', {icon: 2});
                        return false;
                    }
                    $.ajax({
                        type: "POST",
                        data: {'changeIds': selected,'issueId': parent.$('#issueId').val(),
                            'triggerId': parent.$('#issueId').val()},
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        url: "/correlation/batchIssueChange",
                        success: function (data) {
                            if (data.status == 'ok') {
